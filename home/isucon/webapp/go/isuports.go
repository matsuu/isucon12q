...
const (
	...
	initializeScript       = "../sql/init_isu2.sh"
...
func billingReportByCompetition(ctx context.Context, tenantID int64, competitonID string) (*BillingReport, error) {
	...
	if err := adminDB.SelectContext(
		ctx,
		&vhs,
		"SELECT player_id, created_at AS min_created_at FROM visit WHERE tenant_id = ? AND competition_id = ?",
		tenantID,
		comp.ID,
...
func playersAddHandler(c echo.Context) error {
	...
	stmt, err := tx.PrepareContext(ctx, "INSERT INTO player (id, tenant_id, display_name, is_disqualified, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)")
	if err != nil {
		return fmt.Errorf("error prepare insert player: %w", err)
	}
	defer stmt.Close()
	...
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("error commit player: %w", err)
	}
...
func competitionScoreHandler(c echo.Context) error {
	...
	tx, err := adminDB.BeginTxx(ctx, nil)
	if err != nil {
		return fmt.Errorf("error begin last_player_score: tenantID=%d, competitionID=%s, %w", v.tenantID, competitionID, err)
	}
	defer tx.Rollback()
	...
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("error commit last_player_score: tenantID=%d, competitionID=%s, %w", v.tenantID, competitionID, err)
	}
...
func competitionRankingHandler(c echo.Context) error {
	...
	if _, err := adminDB.ExecContext(
		ctx,
		"INSERT INTO visit (tenant_id, competition_id, player_id, created_at) VALUES (?, ?, ?, ?)",
